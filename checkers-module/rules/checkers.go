...
func (game *Game) Move(src, dst Pos) (captured Pos, err error) {
	...
	err = nil
	if !game.PieceAt(src) {
		return NO_POS, fmt.Errorf(fmt.Sprintf("No piece at source position: %v", src))
	}
	if game.PieceAt(dst) {
		return NO_POS, fmt.Errorf(fmt.Sprintf("Already piece at destination position: %v", dst))
	}
	if !game.TurnIs(game.Pieces[src].Player) {
		return NO_POS, fmt.Errorf(fmt.Sprintf("Not %v's turn", game.Pieces[src].Player))
	}
	if !game.ValidMove(src, dst) {
		return NO_POS, fmt.Errorf(fmt.Sprintf("Invalid move: %v to %v", src, dst))
	}
	if game.ValidJump(src, dst) {
...
func Parse(s string) (*Game, error) {
	if len(s) != BOARD_DIM*BOARD_DIM+(BOARD_DIM-1) {
		return nil, fmt.Errorf(fmt.Sprintf("invalid board string: %v", s))
	}
	pieces := make(map[Pos]Piece)
	...
	for y, row := range strings.Split(s, ROW_SEP) {
		for x, c := range strings.Split(row, "") {
			if x >= BOARD_DIM || y >= BOARD_DIM {
				return nil, fmt.Errorf(fmt.Sprintf("invalid board, piece out of bounds: %v, %v", x, y))
			}
			if piece, ok := ParsePiece(c); !ok {
				return nil, fmt.Errorf(fmt.Sprintf("invalid board, invalid piece at %v, %v", x, y))
